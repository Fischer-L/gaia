window.chrome = function() {
		...
		preferencesDB.initializeDefaults(function() {
			...
			$("#searchForm").bind('submit', function() {
				if(isSpinning()) {
					...
					if($("#resultList .searchItem").length > 0) {
						var parent = $("#resultList .searchItem:first").parents(".listItemContainer");
	...
	function loadFirstPage() {
		...
		*/
			...
			wikipediaZeroMessageUrl = window.PROTOCOL + '://' + lang + '.m.' + window.PROJECTNAME + '.org/w/api.php?action=zeroconfig&type=message&agent=ffos-wikipedia',
			wikipediaZeroShownDateTime = localStorage['wikipediaZeroShownDateTime'],
			dateDifference,
			showZeroInSearch = function() { $('#searchParam').attr('placeholder', mw.msg('zero-search-hint' ).toString()); };

		if (wikipediaZeroShownDateTime) {
			dateDifference = Date.now() - wikipediaZeroShownDateTime;
		}

		if (dateDifference && dateDifference < 1000 * 60 * 15) {
			/*
			Allow the page load time to start before changing searchbar.
			The Ajax else{} below does so by its own nature.
			*/
			setTimeout( function() { showZeroInSearch(); }, 200)
		} else {
			/* More than 15 minutes has passed since W0 on, or W0 was never on. */
			$.ajax({
				...
				error: function(error) {
					console.log('Error obtaining W0 status:' + error.message);
				}
			});
		}
