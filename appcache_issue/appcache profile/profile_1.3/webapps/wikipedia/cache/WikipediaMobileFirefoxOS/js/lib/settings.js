window.appSettings = function() {
	...
	function setSelectText( $select ) {
		var upperOptionTextWidth = Math.min(35, Math.floor( $( window ).width() / 9));
		$select.parent()
			.append(
				$( '<span />' )
					.addClass( 'selectText' )
					.text( $select.children( 'option:selected' ).text().substring( 0, upperOptionTextWidth )
						.concat($select.children( 'option:selected' ).text().length > upperOptionTextWidth ? '...' : '')
					)
			).append(
				$( '<br />' )
					.css( 'clear', 'both' )
